...
function autokill() {
    ...
    echo -e ""
    echo -e "[1]  Turn On AutoKill/MultiLogin"
    ...
    1)
        ...
        clear
        echo -e "\033[0;34m━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\033[0m"
        echo -e "\E[44;1;39m        AUTOKILL CONFIGURATION      \E[0m"
        echo -e "\033[0;34m━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\033[0m"
        read -p "Multilogin Maximum Number Allowed: " max
        echo -e ""
        read -p "Enter AutoKill Frequency (in minutes, e.g., 1, 5, 10, 15): " ckill
        echo -e ""
        read -p "Enter Ban Duration (in minutes, e.g., 5, 15, 30, 60): " bduration

        # Convert ban duration to seconds
        ban_seconds=$((bduration * 60))

        # Create the cron job
        echo >/etc/cron.d/tendang
        echo "# Autokill" >/etc/cron.d/tendang
        echo "*/$ckill * * * *  root /usr/bin/tendang $max $ban_seconds" >>/etc/cron.d/tendang && chmod +x /etc/cron.d/tendang
        echo "" >/root/log-limit.txt
        echo -e ""
        echo -e "======================================"
        echo -e ""
        echo -e "      Allowed MultiLogin : $max"
        echo -e "      AutoKill Every     : $ckill Minutes"
        echo -e "      Ban Duration       : $bduration Minutes"
        echo -e ""
        echo -e "======================================"
...
function view_banned_users() {
    clear
    echo -e "\033[0;34m━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\033[0m"
    echo -e "\E[44;1;39m            BANNED USERS           \E[0m"
    echo -e "\033[0;34m━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\033[0m"
    ...
    if [ -f "/root/banned-users.txt" ] && [ -s "/root/banned-users.txt" ]; then
        ...
        while IFS= read -r line || [ -n "$line" ]; do
            ...
            if [ -z "$ban_duration" ]; then
                ban_duration=3600 # Default to 1 hour
            fi
...
function unban_user() {
    ...
    if [[ -n "$UNBAN_USER" ]]; then
        ...
        if grep -q "^$UNBAN_USER " /root/banned-users.txt; then
            ...
            # Also reset their violation count if they have one
            if [ -f "/root/violation-counts.txt" ] && grep -q "^$UNBAN_USER " /root/violation-counts.txt; then
                grep -v "^$UNBAN_USER " /root/violation-counts.txt >/tmp/violation-counts.tmp
                mv /tmp/violation-counts.tmp /root/violation-counts.txt
            fi
...
function unban_all_users() {
    ...
    banned_count=$(wc -l </root/banned-users.txt)
...
# Function to display user violation counts
function view_violations() {
    clear
    echo -e "\033[0;34m━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\033[0m"
    echo -e "\E[44;1;39m        USER VIOLATION COUNTS       \E[0m"
    echo -e "\033[0;34m━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\033[0m"
    echo -e "Username                  Violations"
    echo -e "\033[0;34m━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\033[0m"

    if [ -f "/root/violation-counts.txt" ] && [ -s "/root/violation-counts.txt" ]; then
        while IFS= read -r line || [ -n "$line" ]; do
            # Skip empty lines
            if [ -z "$line" ]; then
                continue
            ...
            ban_status="Not Currently Banned"
            if grep -q "^$user " /root/banned-users.txt; then
                ban_status="CURRENTLY BANNED"
            fi

            # Format and print the line
            printf "%-25s %-10s  %s\n" "$user" "$count" "$ban_status"
        done </root/violation-counts.txt

        echo -e ""
    else
        echo -e " No violation counts found"
    fi

    echo -e ""
    echo -e "\033[0;34m━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\033[0m"
    echo -e ""
    echo -e "[1] Reset All Violation Counts"
    echo -e "[2] Reset Specific User's Violations"
    ...
    case $option in
    1)
        echo -e "Are you sure you want to reset ALL violation counts? (y/n)"
        read confirm
        if [[ "$confirm" =~ [Yy] ]]; then
            >/root/violation-counts.txt
            echo -e "All violation counts have been reset!"
            echo "$(date): Admin reset all violation counts" >>/root/log-limit.txt
        else
            echo -e "Operation cancelled"
        fi
        ;;
    2)
        echo -e ""
        read -p "Enter username to reset violations: " username
        if [ -n "$username" ]; then
            if grep -q "^$username " /root/violation-counts.txt; then
                grep -v "^$username " /root/violation-counts.txt >/tmp/violations-temp.txt
                mv /tmp/violations-temp.txt /root/violation-counts.txt
                echo -e "Violations for user $username have been reset!"
                echo "$(date): Admin reset violations for $username" >>/root/log-limit.txt
            else
                echo -e "User $username has no recorded violations."
            fi
        else
            echo -e "No username specified. No action taken."
        fi
...
echo -e "     ${BICyan}[${BIWhite}9${BICyan}] View Banned Users"
echo -e "     ${BICyan}[${BIWhite}10${BICyan}] Unban User"
echo -e "     ${BICyan}[${BIWhite}11${BICyan}] Unban All Users"
echo -e "     ${BICyan}[${BIWhite}12${BICyan}] View User Violations"
echo -e " ${BICyan}└─────────────────────────────────────────────────────┘${NC}"
